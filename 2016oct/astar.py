# A* Shortest Path Algorithm http://en.wikipedia.org/wiki/A* FB - 201012256
...
class Node:
    ...
    def estimate(self, xDest, yDest):
        ...
        # Manhattan distance d = abs(xd) + abs(yd) Chebyshev distance d = max(abs(xd), abs(yd))
        return(d)


# A-star algorithm. The path returned will be a string of digits of directions.
def pathFind(the_map, n, m, dirs, dx, dy, xA, yA, xB, yB):
    ...
    while len(pq[pqi]) > 0:
        ...
        if x == xB and y == yB:
            # generate the path from finish to start by following the dirs
            path = ''
