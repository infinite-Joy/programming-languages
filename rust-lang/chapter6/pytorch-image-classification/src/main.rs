...
fn main() -> failure::Fallible<()> {
    ...
    for epoch in 1..150 {
        ...
        for (bimages, blabels) in image_dataset.train_iter(64).shuffle().to_device(vs.device()) {
        ...
        let test_accuracy =
            net.batch_accuracy_for_logits(&image_dataset.test_images, &image_dataset.test_labels, vs.device(), 512);
